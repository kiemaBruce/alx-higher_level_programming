...
class Rectangle:
    ...
    Defines a rectangle
    Args:
        number_of_instances (int): the number of class objects that have been
        created.
    """
    ...
    def __repr__(self):
        """Returns a string representation of the object that can be evaluated
        using eval to create the object.
        """

        ret_s = f"Rectangle({self.__width}, {self.__height})"
        return ret_s

    def __del__(self):
        """Executes each time object is about to be destroyed"""
        print("Bye rectangle...")
        self.__class__.number_of_instances -= 1
